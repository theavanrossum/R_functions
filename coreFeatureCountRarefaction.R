#'Calculates the number of "core features" observed (features with presence
#'in all samples) if a subset of the total samples is considered. 
#'Repeats this calculation for each subset (niter) 
#'and for a series of increasingly large subsets
...
coreFeatureCountRarefaction <- function(
  ...
  while(subsampSize <= nsamples){
    ...
    for( i in 1:niter){
      ...
      # get the number of core PCs (with non-zero abundance in all samples)
      pcCount <- sum(rowSums(mpaSub != 0) == ncol(mpaSub))
